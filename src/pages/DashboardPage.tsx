...
const DashboardPage = () => {
  ...
  return (
    <div>
      ...
      {role === 'PRODUCER' && <ProducerDashboard orders={orders} ledger={ledger} userId={user.id} />}
      {role === 'PROCESSOR' && <ProcessorDashboard orders={orders} ledger={ledger} userId={user.id} />}
      {role === 'LOGISTICS' && <LogisticsDashboard orders={orders} ledger={ledger} userId={user.id} />}
      {role === 'RETAILER' && <RetailerDashboard orders={orders} userId={user.id} />}
    </div>
  );
};

// ─── ADMIN ────────────────────────────────────────────
const AdminDashboard = ({ orders, ledger }: { orders: any[]; ledger: any[] }) => {
  const totalGMV = orders.reduce((sum, o) => sum + o.total_value, 0);
  const totalCommission = orders.reduce((sum, o) => sum + (o.total_value * o.commission_percent / 100), 0);
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <MetricCard label="Total GMV" value={`₹${(totalGMV / 1000).toFixed(1)}K`} sub="This month" icon={<TrendingUp className="h-5 w-5" />} trend="up" />
        <MetricCard label="Commission Earned" value={`₹${(totalCommission / 1000).toFixed(1)}K`} sub={`Avg ${totalGMV > 0 ? (totalCommission / totalGMV * 100).toFixed(1) : 0}%`} icon={<CreditCard className="h-5 w-5" />} trend="neutral" />
        <MetricCard label="Active Partners" value={7} sub="2 producers, 2 processors" icon={<Users className="h-5 w-5" />} />
      </div>
...
// ─── PRODUCER ─────────────────────────────────────────
const ProducerDashboard = ({ orders, ledger, userId }: { orders: any[]; ledger: any[]; userId: string }) => {
  ...
  const confirmed = myOrders.filter(o => ['RAW_CONFIRMED', 'PROCESSOR_ASSIGNED', 'PROCESSING_STARTED', 'PROCESSING_COMPLETED', 'LOGISTICS_ASSIGNED', 'IN_TRANSIT', 'DELIVERED', 'SETTLED'].includes(o.status));
  const myLedger = ledger.filter(e => e.actor_id === userId);
  const totalEarned = myLedger.filter(e => e.status === 'RELEASED').reduce((s, e) => s + e.net_amount, 0);
  const pendingPayout = myLedger.filter(e => e.status !== 'RELEASED').reduce((s, e) => s + e.net_amount, 0);
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <MetricCard label="Total Earned" value={`₹${(totalEarned / 1000).toFixed(1)}K`} sub="Released payments" icon={<CreditCard className="h-5 w-5" />} trend="up" />
        <MetricCard label="Pending Payout" value={`₹${(pendingPayout / 1000).toFixed(1)}K`} sub="Awaiting settlement" icon={<Clock className="h-5 w-5" />} trend="neutral" />
      </div>
      ...
      {awaitingConfirmation.length > 0 && (
        <div className="bg-warning/5 border border-warning/20 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            You have {awaitingConfirmation.length} order{awaitingConfirmation.length > 1 ? 's' : ''} waiting for raw material confirmation.
          </p>
        </div>
      )}
...
// ─── PROCESSOR ────────────────────────────────────────
const ProcessorDashboard = ({ orders, ledger, userId }: { orders: any[]; ledger: any[]; userId: string }) => {
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <MetricCard label="In Processing" value={inProcessing.length} sub={`${myOrders.length} total assigned`} icon={<Droplets className="h-5 w-5" />} trend={inProcessing.length > 0 ? 'up' : 'neutral'} />
        <MetricCard label="Completed" value={completed.length} sub="Processing done" icon={<CheckCircle className="h-5 w-5" />} />
        <MetricCard label="Total Earned" value={`₹${(totalEarned / 1000).toFixed(1)}K`} sub="Released payments" icon={<CreditCard className="h-5 w-5" />} trend="up" />
        <MetricCard label="Pending Payout" value={`₹${(pendingPayout / 1000).toFixed(1)}K`} sub="Awaiting settlement" icon={<Clock className="h-5 w-5" />} trend="neutral" />
      </div>
      ...
      {inProcessing.length > 0 && (
        <div className="bg-info/5 border border-info/20 rounded-lg p-4 mb-6">
          <h3 className="text-sm font-medium text-info flex items-center gap-2">
            <Package className="h-4 w-4" /> Active Processing
          </h3>
          <p className="text-sm text-muted-foreground mt-1">
            {inProcessing.length} order{inProcessing.length > 1 ? 's' : ''} currently in your processing queue.
          </p>
        </div>
      )}
...
// ─── LOGISTICS ────────────────────────────────────────
const LogisticsDashboard = ({ orders, ledger, userId }: { orders: any[]; ledger: any[]; userId: string }) => {
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <MetricCard label="Delivered" value={delivered.length} sub="Successfully completed" icon={<CheckCircle className="h-5 w-5" />} />
        <MetricCard label="Total Earned" value={`₹${(totalEarned / 1000).toFixed(1)}K`} sub="Released payments" icon={<CreditCard className="h-5 w-5" />} trend="up" />
      </div>
      ...
      {(awaitingPickup.length > 0 || inTransit.length > 0) && (
        <div className="bg-accent/5 border border-accent/20 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {awaitingPickup.length} awaiting pickup, {inTransit.length} in transit.
          </p>
        </div>
      )}
...
// ─── RETAILER ─────────────────────────────────────────
const RetailerDashboard = ({ orders, userId }: { orders: any[]; userId: string }) => {
  ...
  const totalSpent = myOrders.reduce((s, o) => s + o.total_value, 0);
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <MetricCard label="Active Orders" value={activeOrders.length} sub="In progress" icon={<Clock className="h-5 w-5" />} trend={activeOrders.length > 0 ? 'up' : 'neutral'} />
        <MetricCard label="Delivered" value={deliveredCount} sub="Received orders" icon={<CheckCircle className="h-5 w-5" />} />
        <MetricCard label="Total Spent" value={`₹${(totalSpent / 1000).toFixed(1)}K`} sub="All time" icon={<TrendingUp className="h-5 w-5" />} />
      </div>
...
// ─── Shared recent orders list ────────────────────────
const RecentOrdersList = ({ orders, emptyMessage }: { orders: any[]; emptyMessage?: string }) => {
  if (orders.length === 0) {
    return (
      <div className="bg-card rounded-lg border border-border p-8 text-center shadow-card">
        <p className="text-muted-foreground">{emptyMessage || 'No orders to display.'}</p>
      </div>
    );
  }
