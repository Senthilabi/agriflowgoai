...
const ActorsPage = () => {
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-8">{MOCK_USERS.length} registered supply chain partners</p>
          ...
          <tbody className="divide-y divide-border">
            {MOCK_USERS.map(u => (
              <tr key={u.id} className="hover:bg-secondary/30 transition-colors">
                ...
                <td className="p-4 text-sm text-muted-foreground">{u.created_at}</td>
              </tr>
            ))}
          </tbody>
        </table>
