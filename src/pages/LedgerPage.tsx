  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <MetricCard label="Total Gross" value={`₹${(totalGross / 1000).toFixed(1)}K`} icon={<CreditCard className="h-5 w-5" />} />
        <MetricCard label="Commission Collected" value={`₹${(totalCommission / 1000).toFixed(1)}K`} sub={`${(totalCommission / totalGross * 100).toFixed(1)}% avg rate`} />
        <MetricCard label="Net Payable" value={`₹${(totalNet / 1000).toFixed(1)}K`} sub={`${released.length} released, ${pending.length} pending`} />
      </div>
          ...
          <tbody className="divide-y divide-border">
            {MOCK_LEDGER.map(entry => (
              <tr key={entry.id} className="hover:bg-secondary/30 transition-colors">
                ...
                <td className="p-4 text-sm text-right">₹{entry.gross_amount.toLocaleString()}</td>
                <td className="p-4 text-sm text-right text-destructive">-₹{entry.commission_deducted.toLocaleString()}</td>
                <td className="p-4 text-sm text-right font-medium">₹{entry.net_amount.toLocaleString()}</td>
